...
class _EventsScreenState extends State<EventsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        leading: InkWell(child: const Icon(IconsManager.backButtonIcon),
        onTap: (){
      ...
      body: LoadingOverlay(
        progressIndicator: const SpinKitSpinningLines(color: ColorsManager.primary,),
  ...
  void loadEventsDetails(int? eventCategoryId) {
    EventsService.getEventsDetails(context,eventCategoryId).then((value) {
      log(value.toString());
      setState(() {
        eventDetails = value!;
        isLoading2 = false;
