...
class SignUpService{
  static Future<SignUpObject?> signUp(BuildContext context, String name,String phone,
      String password,String otpCode) async {
    log("LOGIN API");
    SharedPreferences prefs = await SharedPreferences.getInstance();

    //log("data " + email+"  "+ password);
    Uri uri = Uri.parse(AppConstants.REGISTER);
    ...
    if (res.statusCode == 200) {
      ...
      if (bodyJson['success'] == false) {
        ...
        SignUpObject user = SignUpObject.fromJson(data);
        log("Sign Up  Succeffully");
        return user;
      }
    } else {
